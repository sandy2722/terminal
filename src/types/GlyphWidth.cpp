...
{
    return CodepointWidthDetector::Singleton().IsWide(glyph);
}
...
{
    CodepointWidthDetector::Singleton().SetFallbackMethod(std::move(pfnFallback));
}
...
{
    CodepointWidthDetector::Singleton().NotifyFontChanged();
}
