...
{
    auto& rowOffsets = row._charOffsets;
    size_t ch = chBeg;
    ...
    for (const auto& s : til::utf16_iterator{ chars })
    {
        ...
        til::at(rowOffsets, colEnd++) = gsl::narrow_cast<uint16_t>(ch);
        if (wide)
        {
            til::at(rowOffsets, colEnd++) = gsl::narrow_cast<uint16_t>(ch | CharOffsetsTrailer);
        }
